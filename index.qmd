...
function forceCarousel(id) {
  ...
  var cols = 3;
  while (cols > 1) {
    var testWidth = cols * 340 + (cols - 1) * gap;
    if (testWidth <= availWidth) break;
    cols--;
  }

  var cardWidth = Math.floor((availWidth - (cols - 1) * gap) / cols);
  var wrapperWidth = cols * cardWidth + (cols - 1) * gap;
  ...
  grid.style.transition = 'transform 0.4s ease, opacity 0.2s ease';
...
function scrollListing(id, dir) {
  ...
  var cardWidth = wrapper._cardWidth || 340;
  var colWidth = cardWidth + (visible > 1 ? gap : 0);
  ...
  grid.style.transform = 'translateX(-' + (positions[id] * colWidth) + 'px)';
}
