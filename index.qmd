...
// Pick card width based on current screen width
function getCardWidth() {
  var vw = window.innerWidth;
  if (vw <= 400) return 260;   // small phones
  if (vw <= 600) return 280;   // larger phones
  return 340;                  // tablet and desktop
}
...
function forceCarousel(id) {
  ...
  // Start at 3 columns, drop down until cards actually fit
  var cols = 3;
  while (cols > 1) {
    var testWidth = cols * cardWidth + (cols - 1) * gap;
...
function scrollListing(id, dir) {
  ...
  if (!grid) return;
  var gap = 24;
  var cardWidth = wrapper._cardWidth || getCardWidth();
