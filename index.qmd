...
(function () {
  ...
  /* ----------------------------------------------------------
     ...
     On touch devices, the first tap on a card reveals the
     hover content (subtitle, categories, lang/date) by adding
     .pu-touched to the link. A second tap on the same already-
     revealed card follows the link normally.
     Tapping anywhere else clears all revealed cards.
     Only active on touch-capable devices (pointer: coarse).
     ---------------------------------------------------------- */
  var puViewportEl = document.getElementById('pu-viewport');
  if (puViewportEl && window.matchMedia('(pointer: coarse)').matches) {

    puViewportEl.addEventListener('click', function (e) {
      var link = e.target.closest('a.quarto-grid-link');
      if (!link) return;

      if (!link.classList.contains('pu-touched')) {
        /* First tap — reveal content, block navigation */
        e.preventDefault();

        /* Clear any other revealed cards in this viewport */
        puViewportEl.querySelectorAll('a.quarto-grid-link.pu-touched').forEach(function (el) {
          el.classList.remove('pu-touched');
        });

        link.classList.add('pu-touched');
      }
      /* Second tap — link.classList already has pu-touched,
         so we do nothing and let the browser follow href normally */
    });

    /* Tapping outside the carousel clears all revealed cards */
    document.addEventListener('click', function (e) {
      if (!puViewportEl.contains(e.target)) {
        puViewportEl.querySelectorAll('a.quarto-grid-link.pu-touched').forEach(function (el) {
          el.classList.remove('pu-touched');
        });
      }
    ...
    /* Also clear when switching tabs */
