...
window.document.addEventListener("DOMContentLoaded", function (_event) {
  ...
  for (const categoryEl of categoryEls) {
    ...
    categoryEl.onclick = (e) => {
      // Always multi-select — no Ctrl/Cmd needed
      selectedCategories.delete(kDefaultCategory); // ← add this
      activateCategory(category);
      setCategoryHash();
    };
  }
  ...
  for (const listingId of listingIds) {
    ...
    if (list) {
      if (
        ...
        // Filter to selected categories, ignoring kDefaultCategory if others selected
        ...
        list.filter(function (item) {
          ...
          if (itemValues.categories !== null) {
            ...
            return itemCategories.some((category) =>
              effectiveCategories.has(category)
            );
          } else {
            return false;
          }
